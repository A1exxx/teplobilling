...
test('S-shell-01: загрузка и монитор периода', async ({ page }) => {
  ...
  await expect(page.getByText('Каркас продукта (фаза Ф0)')).toBeVisible()
})
...
test('S-shell-04: кнопка демо-данных', async ({ page }) => {
  const button = page.getByTestId('seed-demo-button')
  await expect(button).toBeEnabled({ timeout: 30_000 })
  await button.click()
  await expect(page.getByText('появятся в фазе Ф1', { exact: false })).toBeVisible()
})
